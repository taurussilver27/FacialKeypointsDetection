...
def warp(img1,img2,alpha,subd1,subd2,read11,read22):
    ...
    size = img1.shape
    for counter in range(len(triangle1)):
        print( counter)
        i = [read22[triangle1[counter][0]][0]  ]
        i.append(read22[triangle1[counter][0]][1] )
        i.append(read22[triangle1[counter][1]][0] )
        i.append(read22[triangle1[counter][1]][1] )
        i.append(read22[triangle1[counter][2]][0] )
        i.append(read22[triangle1[counter][2]][1] )
        ...
        for x in range(int(rect[0]),int(rect[2])+1):
            for y in range(int(rect[1]),int(rect[3])+1):
                ...
                if inSide(x,y,i[0],i[1],i[2],i[3],i[4],i[5]):
                        ...
                        # try:
                        imageFinal[y][x] = (img1[p1y1][p1x1]).astype(dtype = np.uint8)    
                        # except IndexError:
                            # pass
